...
builder.Services.AddDbContext<AppDbContext>(options =>
    options.UseSqlite(builder.Configuration["DB_CONNECTION_STRING"]));
...
// Auto-create database file and seed records at launch
using (var scope = app.Services.CreateScope())
{
    var db = scope.ServiceProvider.GetRequiredService<AppDbContext>();
    db.Database.Migrate(); // Applies any pending migrations and creates db file if needed

    // Seed Profile
    if (!db.Profiles.Any())
    {
        db.Profiles.Add(new ProfileEntity
        {
            Id = 1,
            Name = "Mehmet",
    ...
    if (!db.Projects.Any())
    {
        db.Projects.AddRange(new List<ProjectEntity>
        {
            new ProjectEntity
            {
                Id = "platar-lpr",
    ...
    if (!db.Articles.Any())
    {
        db.Articles.AddRange(new List<ArticleEntity>
        {
            new ArticleEntity
            {
            ...
            new ArticleEntity
            {
                ...
                Excerpt_EN = "Handshakes, reconnections, scale-out clustering, and telemetry streams under SignalR hubs.",
            ...
            new ArticleEntity
            {
                ...
                Excerpt_DE = "Analyse der pgvector-Indizes (HNSW und IVFFlat) hinsichtlich Performance und Recall.",
                ImageUrl = "assets/blog_placeholder.png",
